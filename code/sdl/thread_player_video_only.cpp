...
static void log_packet(const AVFormatContext *fmt_ctx, const AVPacket *pkt, const char *tag)
{
    ...
    printf("%s: pts:%s pts_time:%s dts:%s dts_time:%s duration:%s duration_time:%s,TimeBase %d/%d\n",
           tag,
...
int video_decode_thread (void *argv){
    static AVFrame *frame = av_frame_alloc();
    static int64_t init_pts=AV_NOPTS_VALUE;
    ...
    while (!*quit){
        ...
        while( ret >= 0){
            ...
            printf("pts: %ld pts_time %ss\n",frame->pts,av_ts2timestr(frame->pts,&is->stream_time_base));

            //这里控制帧率
            if (init_pts==AV_NOPTS_VALUE){
                init_pts=frame->pts;
            }
            int64_t pts=frame->pts-init_pts;
...
//            }
            ...
            if (frame_pts>system_pts){
                int ms=(frame_pts-system_pts)/1000;
//                if (ms>100)
                    printf("%dms\n",ms);
//                printf("%d\n",(frame_pts-system_pts)/1000000);
